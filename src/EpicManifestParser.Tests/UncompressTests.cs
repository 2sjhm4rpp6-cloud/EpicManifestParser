...
public class UncompressTests
{
    ...
    {
        byte[] chunkBuffer = await File.ReadAllBytesAsync("files/chunk_compressed.bin", TestContext.Current.CancellationToken);
        byte[] uncompressPoolBuffer = ArrayPool<byte>.Shared.Rent(10000000);

...
public class ManifestInfo
{
        ...
        Predicate<ManifestInfoElementDownload>? elementDownloadPredicate = null, CancellationToken cancellationToken = default)
    {
        ...
        if (options.ManifestCacheDirectory is not null)
        {
            ...
            {
                byte[] manifestBuffer = await File.ReadAllBytesAsync(cachePath, cancellationToken).ConfigureAwait(false);
            ...
            {
                ReadOnlySpan<char> span = uri.OriginalString.AsSpan();
...

        {
            ...
            if (elementDownload.QueryParams is { Count: not 0 })
            {
                ...
                foreach (ManifestInfoElementDownloadQueryParams queryParam in elementDownload.QueryParams)
                {
...
public static partial class ManifestExtensions
{
    ...
    public static bool TryParseVersionAndCL(string buildVersion, [NotNullWhen(true)] out Version? version, out int cl)
    {
        ...
        Match match = VersionAndClRegex().Match(buildVersion);
        if (!match.Success)
            return false;
